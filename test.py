...
def find_nearest_upper_air_station(
    ...
    for sounding_time in (
        candidate_standard_times()
    ):
    ...
    raise RuntimeError(
        "Unable to determine nearest upper-air station. "
...
def load_latest_sounding(
    ...
    raise RuntimeError(
        f"No recent sounding available for "
        f"{station}. Last error: {last_error}"
    )
...
def build_environment_signals(
    ...
    return {
        ...
        "strong_deep_layer_shear":
            (
                shear_6km is not None
                and shear_6km >= 35
            ),
...
class CurrentSkewTInput(
    ...
    station: str | None = Field(
        default=None,
        description=(
            "Optional upper-air station identifier. "
            "Normally leave this empty and the nearest "
            "available radiosonde station will be selected."
        ),
    )
...
def analyze_current_skewt(
    ...
    """
    Analyze the latest observed radiosonde sounding near a location.

    Use this tool when the user asks about the current Skew-T,
    observed atmospheric sounding, instability, CAPE/CIN,
    lapse rates, LCL/LFC/EL, precipitable water, freezing level,
    wind shear, or the vertical thermodynamic environment.

    This tool uses observed upper-air sounding data rather than
    HRRR model guidance.

    Radiosonde observations are normally available around 00Z
    and 12Z, with occasional special soundings at other times.
    Therefore the result represents the latest observed vertical
    profile, not instantaneous conditions.

    Do not use this tool alone to declare that severe weather
    is occurring. It describes the observed atmospheric
    environment at the sounding location and time.
    """
    ...
    if station is None:
        ...
        station_info = (
            find_nearest_upper_air_station(
                latitude,
                longitude,
            )
        )
    ...
    # ========================================================
    # Load newest sounding
    # ========================================================

    df, sounding_time = (
        load_latest_sounding(
            station_id
        )
    )

    # ========================================================
    # Get station coordinates if needed
    # ========================================================
    ...
    return {
        "source":
            "Observed radiosonde via Iowa Environmental Mesonet",
        ...
        "observation": {
            ...
            "standard_or_special_time":
                (
                    "standard"
                    if sounding_time.hour
                    in (
                        0,
                        12,
                    )
        ...
        "limitations": [
            (
                "This is an observed radiosonde profile "
                "from the nearest available upper-air station, "
                "not necessarily directly over the requested location."
            ),

            (
                "The sounding describes the atmosphere at its "
                "observation time and may be several hours old."
            ),
...
import json
print(json.dumps(analyze_current_skewt(40.4406, -79.9959), indent=4))