...
| EventBridge Scheduler schedule | Hourly 8am–8pm ET plus 4am overnight (`infra/eventbridge.tf`) |
...
```text
cron(0 4,8-20 * * ? *)   → 4am, then hourly 8am–8pm (US Eastern)
```

That is **hourly between 8am and 8pm**, plus a **4am overnight** run (8pm → 4am → 8am). Keep `briefing.schedule_hours` in `config.yaml` in sync so markdown **Updated** / **Next update** headers match.
...
| Schedule | Manual (`python main.py` or `python main.py --local`) | EventBridge Scheduler — hourly 8am–8pm ET + 4am overnight |
